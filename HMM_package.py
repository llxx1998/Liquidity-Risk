"""
This file contains classes and functions from Oleg Zeros Word documents.
The difference between this and our model is that our emission matrix is continuous.
"""
...
class HiddenMarkovChain:
    ...
    def _create_all_chains(self, chain_length):  # generate all path for the same steps
...
class HiddenMarkovChain_FP(HiddenMarkovChain):
    def _alphas(self, observations: list) -> np.ndarray:
        ...
        alphas[0, :] = self.pi.values * self.E[observations[0]].T  # outer product
        for t in range(1, len(observations)):
            alphas[t, :] = (alphas[t - 1, :].reshape(1, -1)
                            @ self.T.values) * self.E[observations[t]].T  # @ means outer product
        return alphas
...
class HiddenMarkovChain_Uncover(HiddenMarkovChain_Simulation):
    ...
    def uncover(self, observations: list) -> list:
        ...
        print(list(map(lambda x: self.states[x], maxargs)))
